...
- `atmosphere.stars` — adds star intensity in the space band.
...
Heights scale linearly to `value / max(values) * max_height_m`. Needs a pitched
camera to read as 3D. Emits a `fill` event.
